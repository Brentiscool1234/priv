 ...
 */
function generateSchema(page, profile, locale) {
    const schemas = [];
    const loc = (0, locales_1.getLocaleConfig)(locale);
...
function buildOrganization(profile, businessName, _lang) {
    const schema = {
    ...
    if (profile.booking_url)
        schema['url'] = profile.booking_url;
    if (profile.description)
        schema['description'] = profile.description;
    return schema;
}
function buildWebSite(page, businessName, _lang) {
...
function buildWebPage(page, businessName, description, _lang) {
    return {
        '@context': 'https://schema.org',
        '@type': 'WebPage',
...
function buildBreadcrumb(page, businessName, _lang) {
...
function buildService(page, businessName, description, _lang) {
    return {
        '@context': 'https://schema.org',
        '@type': 'Service',
...
function buildLocalBusiness(profile, businessName, description, _lang, page) {
    const schema = {
        '@context': 'https://schema.org',
        '@type': 'LocalBusiness',
        name: businessName,
    ...
    if (profile.booking_url)
        schema['hasMap'] = profile.booking_url;
...
function buildFAQPage(page, _lang) {
