 ...
 */
function generateBrief(ctx) {
    const { plan, businessProfile, service, location, allPlans } = ctx;
    const locale = plan.locale;
    const localeConfig = (0, locales_1.getLocaleConfig)(locale);
    const template = (0, templates_1.getTemplate)(plan.page_type);
    const businessName = businessProfile.business_name ?? 'Our Business';
    const now = new Date().toISOString();
    const cityName = location
        ? (0, locale_engine_1.getLocalizedCityName)(location.city, locale)
        : undefined;
    ...
    const metaDescription = buildMetaDescription(plan, primaryKeyword, cityName, businessName, locale, localeConfig.cta_style);
...
// ─── Keyword Builders ────────────────────────────────────────────────────────
function buildPrimaryKeyword(plan, service, cityName, locale) {
...
function buildH1(plan, primaryKeyword, cityName, businessName, locale) {
    switch (plan.page_type) {
        case 'homepage':
            return businessName;
        case 'about':
...
function buildMetaTitle(plan, primaryKeyword, cityName, businessName, _locale) {
    const max = 60;
    let title;
    switch (plan.page_type) {
        case 'homepage':
...
function buildMetaDescription(plan, primaryKeyword, cityName, businessName, locale, ctaStyle) {
    let desc;
    ...
    if (desc.length < 140)
        desc = desc + (' ').repeat(0); // leave short ones as-is
    return desc;
}
function buildSecondaryKeywords(plan, service, cityName, _locale) {
...
function buildFaqQuestions(plan, service, cityName, businessName, locale) {
    const lang = (0, locales_1.getLocaleConfig)(locale).language;
...
function buildInternalLinks(plan, allPlans, _locale) {
    const links = [];
    ...
    switch (plan.page_type) {
        ...
        case 'service_location': {
            ...
            const parentSvc = sameLocale.find((p) => p.page_type === 'service' && p.service_id === plan.service_id);
            const parentLoc = sameLocale.find((p) => p.page_type === 'location' && p.location_id === plan.location_id);
            if (parentSvc) {
...
function buildCtaAngle(plan, ctaStyle, cityName, _locale) {
...
function contactLabel(locale) {
    const lang = locale ? (0, locales_1.getLocaleConfig)(locale).language : 'en';
    const map = { en: 'Contact Us', nl: 'Contact', fr: 'Contactez-Nous', de: 'Kontakt' };
    return map[lang] ?? 'Contact Us';
}
function faqLabel(locale) {
    const lang = locale ? (0, locales_1.getLocaleConfig)(locale).language : 'en';
    const map = {
...
function aboutH1(businessName, locale) {
    const lang = (0, locales_1.getLocaleConfig)(locale).language;
    const map = {
...
function localH1(cityName, locale) {
    const lang = (0, locales_1.getLocaleConfig)(locale).language;
    const map = {
...
function metaDescByLang(lang, map) {
