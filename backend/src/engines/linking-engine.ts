import type { PagePlan, InternalLink, PageType } from '../types';
...
export function computeInternalLinks(
  ...
  function addLink(slug: string, anchor: string, pageType: PageType): void {
  ...
  if (homepage) addLink(homepage.slug, homeLabel(lang), 'homepage');
  if (contact) addLink(contact.slug, contactLabel(lang), 'contact');
  ...
  switch (page.page_type) {
    ...
    case 'location': {
      ...
      const cityServices = sameLocale.filter(
        (p) =>
          p.page_type === 'service_location' &&
    ...
    case 'service_location': {
      ...
      const parentSvc = sameLocale.find(
        (p) => p.page_type === 'service' && p.service_id === page.service_id
      );
      if (parentSvc) {
        const svcPart = parentSvc.slug.split('/').pop() ?? parentSvc.slug;
        addLink(
          parentSvc.slug,
          capitalize(svcPart.replace(/-/g, ' ')),
          'service'
        );
      }
      ...
      const parentLoc = sameLocale.find(
        (p) => p.page_type === 'location' && p.location_id === page.location_id
      );
      if (parentLoc) {
        const cityPart = parentLoc.slug.split('/').pop() ?? parentLoc.slug;
        addLink(
          parentLoc.slug,
          servicesInCityLabel(cityPart, lang),
          'location'
        );
      }
    ...
    case 'about': {
      ...
      if (faq) addLink(faq.slug, faqLabel(lang), 'faq');
    ...
    case 'contact': {
      ...
      if (about) addLink(about.slug, aboutLabel(lang), 'about');
...
function homeLabel(lang: string): string {
  const map: Record<string, string> = { en: 'Home', nl: 'Home', fr: 'Accueil', de: 'Startseite' };
  return map[lang] ?? 'Home';
}

function contactLabel(lang: string): string {
...
function faqLabel(lang: string): string {
  const map: Record<string, string> = {
    en: 'FAQ',
...
function aboutLabel(lang: string): string {
  const map: Record<string, string> = {
    en: 'About Us',
    nl: 'Over Ons',
    fr: 'À Propos',
...
function localServiceAnchor(citySlug: string, lang: string): string {
...
function servicesInCityLabel(citySlug: string, lang: string): string {
  const city = capitalize(citySlug.replace(/-/g, ' '));
  const map: Record<string, string> = {
    en: `Services in ${city}`,
    nl: `Diensten in ${city}`,
