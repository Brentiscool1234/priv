...
export function runQAChecks(page: any, allSlugs: string[], allPages: any[]): QACheckResult[] {
  ...
  if (page.meta_title) {
    ...
    if (len > 60) results.push({ check_name: 'meta_title_length', status: 'warn', message: `Meta title is ${len} chars (recommended: ≤60)` });
    else if (len < 30) results.push({ check_name: 'meta_title_length', status: 'warn', message: `Meta title is only ${len} chars (recommended: ≥30)` });
    else results.push({ check_name: 'meta_title_length', status: 'pass' });
  }
  ...
  if (page.meta_description) {
    ...
    if (len > 160) results.push({ check_name: 'meta_description_length', status: 'warn', message: `Meta description is ${len} chars (recommended: ≤160)` });
    else if (len < 120) results.push({ check_name: 'meta_description_length', status: 'warn', message: `Meta description is ${len} chars (recommended: ≥120)` });
    else results.push({ check_name: 'meta_description_length', status: 'pass' });
  }
  ...
  // faq_schema_valid
  const faqSchema = Array.isArray(schemas) ? schemas.find((s: any) => s['@type'] === 'FAQPage') : null;
  if (faqSchema) {
  ...
  const dupTitles = allPages.filter((p: any) => p.id !== page.id && p.meta_title && p.meta_title === page.meta_title);
  ...
  if (pageH1) {
    const dupH1 = allPages.filter((p: any) => p.id !== page.id && p.h1 === pageH1);
  ...
  const imgsWithoutAlt = imgTags.filter((img: string) => !img.includes('alt='));
  if (imgTags.length > 0) {
